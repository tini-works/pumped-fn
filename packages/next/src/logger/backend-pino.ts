...
const importPino = (): Promise<(options: Record<string, unknown>) => PinoLogger> => {
  return new Promise(async (resolve, reject) => {
    try {
      const moduleName = 'pino'
      const pinoModule = await import(moduleName)
      const pinoConstructor = pinoModule.default || pinoModule
      resolve(pinoConstructor)
    } catch (error) {
      reject(new Error('Pino package not installed. Install with: npm install pino pino-pretty pino-roll'))
    }
  })
}

export const pinoLogger: Core.Executor<Logger.Instance> = provide(async ({ scope }): Promise<Logger.Instance> => {
